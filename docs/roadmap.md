...
### FR-031 — Notifications (completed: generic outbound webhook; email and per-repository configuration remain future work)
A run reaching a state needing human attention (`AWAITING_APPROVAL`, `SUCCEEDED`, `FAILED`, `PUBLISH_FAILED`) raises an in-app toast wherever the dashboard is open, and can also POST one JSON payload — a Slack-incoming-webhook-compatible `text` field plus structured `runId`/`runName`/`repositoryId`/`state` fields — to a single configured webhook URL, with an optional deep link back to the run. Off by default; async and best-effort by design (never adds pipeline latency, never retries) — see [ADR-0016](adr/0016-outbound-run-notifications.md). Not yet built: email delivery, per-repository/team configuration, and rich Slack Block Kit/Teams Adaptive Card formatting.
...
Closes the gap FR-027 left for specs: `contractguard.specs.directory` was the only source, requiring filesystem or deployment access to offer a user's own OpenAPI files. Adds a second, read-only registration kind — a spec-source repository, reusing FR-027's clone/credential machinery but a separate registry/table, never a `WorkspacePolicy` root — with an **optional** token, since most spec repositories are public; and a direct upload endpoint for users with spec files but no repository for them. All three origins (bundled, uploaded, spec-source) land in one combined picker via a qualified spec ID (`local:`/`upload:`/`source:<id>:`), with a bare file name still resolving locally for backward compatibility with the FR-026 CLI gate and already-persisted runs — see [ADR-0012](adr/0012-spec-source-repositories-and-upload.md). The dashboard's setup form (a 3-step wizard when this shipped) was superseded by FR-044.

### FR-044 — Source Deregistration and Manage-Sources UX (completed)
Registering was one-way: a repository or spec source added with the wrong URL or an under-scoped token stayed listed forever, and the only fix was a database edit. Adds `DELETE` endpoints for all three registration kinds (remote repository, spec source, uploaded specification); removing a remote repository or spec source also deletes its local clone cache, so a corrected re-registration never reuses a stale checkout. Also replaces the FR-043 setup wizard with a single-panel "New analysis run" form (pick repository, old spec, new spec, name, go — no forward/back navigation) plus a separate **Manage sources** modal for all registration, upload and removal actions, so starting a run and curating sources are two deliberately distinct actions rather than one wizard. A first-run onboarding banner and an in-app quick-reference panel round out the UX for a first-time operator. (The Manage sources modal later became the multi-page app's **Settings** page — same separation, now a routed page alongside the statistics dashboard and run history.)

### FR-045 — Deterministic Spec-Diff Preview (completed)
Starting a run was the only way to see whether two specification versions even differ. The New Run form now shows an instant, zero-cost preview — classified changes and a breakdown by breaking/non-breaking, with an expandable detail table — the moment both specifications are selected, computed by reusing the exact same deterministic diff step (`OpenApiDiffPort`/`SpecDiffEngine`, classification already included) a real run computes first. No run is created, no repository is touched, no LLM is called, so it can never disagree with what a real run detects. See [ADR-0017](adr/0017-deterministic-spec-diff-preview.md).

### FR-046 — "Run Again" (completed)
Re-running the same repository and specification pair — e.g. after fixing whatever caused a rejection or failure — required manually reselecting everything on New Run. A **Run again** button on the run detail page now carries the exact qualified specification IDs (not just their display names, which can collide across origins) and the repository ID to New Run via navigation state, pre-filling the form; a carried-over ID that's no longer valid (its source was deregistered since) falls back to the form's normal defaults rather than silently failing.
...
Shipped: parameter add/remove/required/type changes, request-body add/remove/schema-change, response status-code add/remove, property nullability changes, numeric/string constraint tightening (`minimum`/`maximum`/`minLength`/`maxLength`), request-body content-type add/remove, and operation-level security-requirement add/remove — fifteen new deterministic `ChangeType` categories replacing the previous generic `UNKNOWN_CHANGE` fallback for operation-level differences (see [ADR-0014](adr/0014-expanded-change-taxonomy.md) and its addendum). Not yet built: regex `pattern` tightening (undecidable in general, a permanent scope boundary rather than a gap to close), per-response-status content types (only the request body's content types are tracked), security scope-list/scheme-definition changes (only which schemes are required per operation is compared), and nullability/constraint tightening for parameters (tracked for schema properties only).
